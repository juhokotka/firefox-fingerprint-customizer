...
class WindowGlobalChild final : public WindowGlobalActor,
 ...
 protected:
  ...
  mozilla::ipc::IPCResult RecvUpdateProfile(const Maybe<ProfileArgs>& aProfile);

  virtual void ActorDestroy(ActorDestroyReason aWhy) override;

public:
  // ── Fingerprint Profile cache (content process) ──────────────────
  // Received Profiles are cached keyed by userContextId so that all RFP
  // interception points can read them with zero IPC on the hot path.
  // Returns true if a Profile was found for the given userContextId.
  static bool GetProfileForUserContextId(uint32_t aUserContextId,
