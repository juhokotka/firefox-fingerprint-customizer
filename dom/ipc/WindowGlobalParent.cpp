...
void WindowGlobalParent::Init() {
  ...
  // If a fingerprint Profile is cached for this container, push it to the
  // content process immediately so RFP interception points have it available
  // before first paint.
  if (CanSend()) {
    uint32_t userContextId = BrowsingContext()->OriginAttributesRef().mUserContextId;
    ProfileArgs cachedProfile;
    if (GetCachedProfile(userContextId, &cachedProfile)) {
      (void)SendUpdateProfile(Some(cachedProfile));
    }
  }
...
// ── Fingerprint Profile: parent-side cache & conversion ─────────────

namespace {

// We wrap the hashmap in a function-local static to avoid static
// destructor ordering issues across translation units.  ProfileArgs is
// stored behind a UniquePtr because its inline size exceeds the 255-byte
// PLDHashTable entry limit.
nsTHashMap<uint32_t, UniquePtr<ProfileArgs>>& ParentProfileCache() {
  static nsTHashMap<uint32_t, UniquePtr<ProfileArgs>> sCache;
  return sCache;
}

// Convert a WebIDL FingerprintScreenDict to an IPDL FingerprintScreenArgs.
FingerprintScreenArgs ConvertScreenDict(const FingerprintScreenDict& aDict) {
  FingerprintScreenArgs args;
  args.width() = aDict.mWidth;
  args.height() = aDict.mHeight;
  args.availWidth() = aDict.mAvailWidth;
  args.availHeight() = aDict.mAvailHeight;
  args.availLeft() = aDict.mAvailLeft;
  args.availTop() = aDict.mAvailTop;
  args.pixelDepth() = aDict.mPixelDepth;
  args.colorDepth() = aDict.mColorDepth;
  return args;
}

// Convert a WebIDL FingerprintMediaDeviceDict to an IPDL FingerprintMediaDeviceArgs.
FingerprintMediaDeviceArgs ConvertMediaDeviceDict(
    const FingerprintMediaDeviceDict& aDict) {
  FingerprintMediaDeviceArgs args;
  args.kind() = NS_ConvertUTF16toUTF8(aDict.mKind);
  args.label() = NS_ConvertUTF16toUTF8(aDict.mLabel);
  return args;
}

// Convert a WebIDL FingerprintDeviceDict to an IPDL FingerprintDeviceArgs.
FingerprintDeviceArgs ConvertDeviceDict(const FingerprintDeviceDict& aDict) {
  FingerprintDeviceArgs args;
  args.name() = NS_ConvertUTF16toUTF8(aDict.mName);
  args.platform() = NS_ConvertUTF16toUTF8(aDict.mPlatform);
  args.userAgent() = NS_ConvertUTF16toUTF8(aDict.mUserAgent);
  args.oscpu() = NS_ConvertUTF16toUTF8(aDict.mOscpu);
  args.appVersion() = NS_ConvertUTF16toUTF8(aDict.mAppVersion);
  args.navigatorPlatform() = NS_ConvertUTF16toUTF8(aDict.mNavigatorPlatform);
  args.hardwareConcurrency() = aDict.mHardwareConcurrency;
  args.maxTouchPoints() = aDict.mMaxTouchPoints;
  args.doNotTrack() = NS_ConvertUTF16toUTF8(aDict.mDoNotTrack);
  args.screen() = ConvertScreenDict(aDict.mScreen);
  args.devicePixelRatio() = aDict.mDevicePixelRatio;
  args.webglVendor() = NS_ConvertUTF16toUTF8(aDict.mWebglVendor);
  args.webglRenderer() = NS_ConvertUTF16toUTF8(aDict.mWebglRenderer);
  args.audioSampleRate() = aDict.mAudioSampleRate;

  // Convert fontSet: sequence<DOMString> → nsCString[]
  nsTArray<nsCString>& fontSet = args.fontSet();
  fontSet.SetCapacity(aDict.mFontSet.Length());
  for (const nsString& font : aDict.mFontSet) {
    fontSet.AppendElement(NS_ConvertUTF16toUTF8(font));
  }

  // Convert mediaDevices: sequence<Dict> → Args[]
  nsTArray<FingerprintMediaDeviceArgs>& devices = args.mediaDevices();
  devices.SetCapacity(aDict.mMediaDevices.Length());
  for (const FingerprintMediaDeviceDict& dev : aDict.mMediaDevices) {
    devices.AppendElement(ConvertMediaDeviceDict(dev));
  }

  return args;
}

// Convert a WebIDL FingerprintLocationDict to an IPDL FingerprintLocationArgs.
FingerprintLocationArgs ConvertLocationDict(
  ...
  languages.SetCapacity(aDict.mLanguages.Length());
  for (const nsString& lang : aDict.mLanguages) {
    languages.AppendElement(NS_ConvertUTF16toUTF8(lang));
  }
  return args;
}

// Convert a WebIDL FingerprintNoiseDict to an IPDL FingerprintNoiseArgs.
FingerprintNoiseArgs ConvertNoiseDict(const FingerprintNoiseDict& aDict) {
...
ProfileArgs ConvertProfileDict(const FingerprintProfileDict& aDict) {
  ...
  // Dictionaries are non-nullable (with defaults). Only set the IPDL
  // nullable fields if the dict has meaningful content.
  if (!aDict.mDevice.mName.IsEmpty()) {
    args.device() = Some(ConvertDeviceDict(aDict.mDevice));
  }
  ...
  args.geolocation() = NS_ConvertUTF16toUTF8(aDict.mGeolocation);

  nsTArray<nsCString>& exts = args.allowedExtensions();
  exts.SetCapacity(aDict.mAllowedExtensions.Length());
  for (const nsString& ext : aDict.mAllowedExtensions) {
    exts.AppendElement(NS_ConvertUTF16toUTF8(ext));
  }
...
/* static */
bool WindowGlobalParent::GetCachedProfile(uint32_t aUserContextId,
                                          ProfileArgs* aOutProfile) {
  auto& cache = ParentProfileCache();
  auto entry = cache.Lookup(aUserContextId);
  if (!entry) {
    return false;
  }
  *aOutProfile = *entry.Data();
  return true;
}

/* static */
void WindowGlobalParent::CacheProfile(uint32_t aUserContextId,
...
void WindowGlobalParent::UpdateProfile(
    const FingerprintProfileDict& aProfile, ErrorResult& aRv) {
  // Only the parent process should receive this WebIDL call.
  MOZ_ASSERT(XRE_IsParentProcess());

  CanonicalBrowsingContext* bc = GetBrowsingContext();
  if (!bc) {
    aRv.ThrowInvalidStateError(
        "Cannot update Profile: BrowsingContext is not available");
    return;
  }

  uint32_t userContextId = bc->OriginAttributesRef().mUserContextId;

  // An empty profile (all defaults) clears the container's Profile.
  if (aProfile.mDevice.mName.IsEmpty()) {
    RemoveCachedProfile(userContextId);
    if (CanSend()) {
      (void)SendUpdateProfile(Maybe<ProfileArgs>());
    }
    return;
  }

  // Convert WebIDL dictionary → IPDL struct.
  ProfileArgs profileArgs = ConvertProfileDict(aProfile);

  // Cache parent-side so new WindowGlobals in this container also get it.
  CacheProfile(userContextId, profileArgs);

  // Push to the content process.
  if (CanSend()) {
    (void)SendUpdateProfile(Some(profileArgs));
  }
}
