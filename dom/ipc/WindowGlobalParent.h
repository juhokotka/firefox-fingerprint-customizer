...
#include "mozilla/dom/FingerprintProfileBinding.h"
...
class WindowGlobalParent final : public WindowContext,
 ...
 public:
  ...
  // ── Fingerprint Profile (parent process) ────────────────────────
  // WebIDL-exposed method: JS calls updateProfile(dict) to push a
  // Profile to the content process. Caches it parent-side so new
  // WindowGlobals in the same container also receive it.
  void UpdateProfile(const FingerprintProfileDict& aProfile, ErrorResult& aRv);

  // Parent-side cache (userContextId → ProfileArgs). Used to push
  // Profiles to newly created WindowGlobals in the same container.
  static bool GetCachedProfile(uint32_t aUserContextId,
                               ProfileArgs* aOutProfile);
  static void CacheProfile(uint32_t aUserContextId,
                           const ProfileArgs& aProfile);
  static void RemoveCachedProfile(uint32_t aUserContextId);
