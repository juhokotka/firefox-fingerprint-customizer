...
namespace {
// Returns the FingerprintDeviceArgs from the cached Profile for the given
// window's container, if profileMode is active and a Profile has been pushed
// for that container. Returns false when no Profile is available (normal mode
// or a container without a Profile), causing callers to fall through to the
// existing RFP behavior.
bool GetProfileDeviceForWindow(nsPIDOMWindowInner* aWindow,
                               FingerprintDeviceArgs* aOut) {
  if (!aWindow) {
    return false;
  }
  ...
  if (profile.device().isNothing()) {
    return false;
  }
  *aOut = profile.device().ref();
  return true;
}
}  // namespace
...
int32_t nsScreen::PixelDepth() {
  ...
  if (ShouldResistFingerprinting(RFPTarget::ScreenPixelDepth)) {
    // profileMode: return Profile's screen pixelDepth if available.
    FingerprintDeviceArgs device;
    if (GetProfileDeviceForWindow(GetOwnerWindow(), &device)) {
      return device.screen().pixelDepth();
    }
    return 24;
  }
...
CSSIntRect nsScreen::GetRect() {
  ...
  if (ShouldResistFingerprinting(RFPTarget::ScreenRect)) {
    // profileMode: return Profile's screen dimensions if available.
    FingerprintDeviceArgs device;
    if (GetProfileDeviceForWindow(GetOwnerWindow(), &device)) {
      const FingerprintScreenArgs& screen = device.screen();
...
CSSIntRect nsScreen::GetAvailRect() {
  ...
  if (ShouldResistFingerprinting(RFPTarget::ScreenAvailRect)) {
    // profileMode: return Profile's available screen rect if available.
    FingerprintDeviceArgs device;
    if (GetProfileDeviceForWindow(GetOwnerWindow(), &device)) {
      const FingerprintScreenArgs& screen = device.screen();
      return {screen.availLeft(), screen.availTop(), screen.availWidth(),
              screen.availHeight()};
    }
    return GetTopWindowInnerRectForRFP();
  }
