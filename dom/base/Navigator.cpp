...
uint64_t Navigator::HardwareConcurrency() {
  ...
  // profileMode: return Profile's hardwareConcurrency if available.
    ...
    if (bc) {
      ...
      if (userContextId != 0) {
        ...
        if (WindowGlobalChild::GetProfileForUserContextId(userContextId,
                                                          &profile) &&
            profile.device().isSome()) {
          return profile.device().ref().hardwareConcurrency();
        }
      }
    }
  }
...
uint32_t Navigator::MaxTouchPoints(CallerType aCallerType) {
  ...
  if (aCallerType != CallerType::System &&
    ...
    // profileMode: return Profile's maxTouchPoints if available.
    if (bc) {
      uint32_t userContextId = bc->OriginAttributesRef().mUserContextId;
      if (userContextId != 0) {
        ProfileArgs profile;
        if (WindowGlobalChild::GetProfileForUserContextId(userContextId,
                                                          &profile) &&
            profile.device().isSome()) {
          return profile.device().ref().maxTouchPoints();
        }
      }
    }
