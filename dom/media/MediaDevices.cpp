...
RefPtr<MediaDeviceSetRefCnt> MediaDevices::FilterExposedDevices(
  ...
  if (doc->ShouldResistFingerprinting(RFPTarget::MediaDevices)) {
    // profileMode: if the container has a Profile with a mediaDevices list,
    // substitute the real device list entirely with synthesized devices built
    // from the Profile entries. Each entry provides a kind and a label.
    ProfileArgs profileStorage;
    FingerprintDeviceArgs profileDevice;
    bool hasProfileDevice = false;
    if (BrowsingContext* bc = window->GetBrowsingContext()) {
      uint32_t userContextId = bc->OriginAttributesRef().mUserContextId;
      if (userContextId != 0 &&
          WindowGlobalChild::GetProfileForUserContextId(userContextId,
                                                        &profileStorage) &&
          profileStorage.device().isSome()) {
        profileDevice = profileStorage.device().ref();
        hasProfileDevice = !profileDevice.mediaDevices().IsEmpty();
      }
    }
    ...
    if (hasProfileDevice) {
      ...
      for (const auto& entry : profileDevice.mediaDevices()) {
        const nsCString& kind = entry.kind();
        const nsCString& label = entry.label();
        nsString wideLabel = NS_ConvertUTF8toUTF16(label);
        if (kind.EqualsLiteral("audioinput")) {
          exposed->AppendElement(new MediaDevice(
              fakeEngine, MediaSourceEnum::Microphone, wideLabel,
              u""_ns, u""_ns, IsScary::No, OsPromptable::No));
        } else if (kind.EqualsLiteral("videoinput")) {
          exposed->AppendElement(new MediaDevice(
              fakeEngine, MediaSourceEnum::Camera, wideLabel,
              u""_ns, u""_ns, IsScary::No, OsPromptable::No));
        } else if (kind.EqualsLiteral("audiooutput")) {
          RefPtr info = new AudioDeviceInfo(
              nullptr, wideLabel, u""_ns, u""_ns, CUBEB_DEVICE_TYPE_OUTPUT,
              CUBEB_DEVICE_STATE_ENABLED, CUBEB_DEVICE_PREF_ALL,
              CUBEB_DEVICE_FMT_ALL, CUBEB_DEVICE_FMT_S16NE, 2, 44100, 44100,
              44100, 128, 128);
          exposed->AppendElement(
              new MediaDevice(fakeEngine, info, u""_ns));
        }
      }
      return exposed;
    }
