...
bool PeerConnectionImpl::GetPrefDefaultAddressOnly() const {
  ...
  // profileMode: force default_address_only on when the container's Profile
  // has webrtcHideIP, hiding the real LAN IP regardless of underlying prefs.
  if (mozilla::StaticPrefs::privacy_fingerprint_profileMode() && mWindow) {
    if (BrowsingContext* bc = mWindow->GetBrowsingContext()) {
      uint32_t userContextId = bc->OriginAttributesRef().mUserContextId;
      if (userContextId != 0) {
        ProfileArgs profile;
        if (WindowGlobalChild::GetProfileForUserContextId(userContextId,
                                                          &profile) &&
            profile.webrtcHideIP()) {
          return true;
        }
      }
    }
...
bool PeerConnectionImpl::GetPrefObfuscateHostAddresses() const {
  ...
  // profileMode: force mDNS host obfuscation on when the container's Profile
  // has webrtcHideIP, overriding blocklist/content-process/safety-valve checks.
  if (mozilla::StaticPrefs::privacy_fingerprint_profileMode() && mWindow) {
    if (BrowsingContext* bc = mWindow->GetBrowsingContext()) {
      uint32_t userContextId = bc->OriginAttributesRef().mUserContextId;
      if (userContextId != 0) {
