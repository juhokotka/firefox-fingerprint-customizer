...
namespace {
// Returns true when profileMode is active and the container's Profile has
// geolocation == "blocked", meaning the request should be denied with
// PERMISSION_DENIED without consulting the platform location provider.
bool ShouldDenyGeolocationByProfile(BrowsingContext* aBC) {
  if (!StaticPrefs::privacy_fingerprint_profileMode() || !aBC) {
    return false;
  }
  uint32_t userContextId = aBC->OriginAttributesRef().mUserContextId;
  if (userContextId == 0) {
    return false;
  }
  ProfileArgs profile;
  ...
  return profile.geolocation().EqualsLiteral("blocked");
}
}  // namespace
...
nsresult Geolocation::GetCurrentPosition(GeoPositionCallback callback,
  ...
  if (!StaticPrefs::geo_enabled() || ShouldBlockInsecureRequests() ||
      ...
      ShouldDenyGeolocationByProfile(mBrowsingContext)) {
