...
void ClientWebGLContext::GetParameter(JSContext* cx, GLenum pname,
  ...
  if (!debug) {
    // profileMode: fetch Profile device for WebGL string spoofing. When a
    // Profile is available, the RFP masking targets (WebGLRenderInfo etc.)
    ...
    const auto GetUnmaskedRenderer = [&]() {
      // profileMode: return Profile's webglRenderer if available.
      if (profileDevice) {
        const auto& renderer = profileDevice->webglRenderer();
        if (!renderer.IsEmpty()) {
    ...
    const auto GetUnmaskedVendor = [&]() {
      // profileMode: return Profile's webglVendor if available.
      if (profileDevice) {
        const auto& vendor = profileDevice->webglVendor();
        if (!vendor.IsEmpty()) {
