...
FontVisibility FontVisibilityProvider::ComputeFontVisibility() const {
  ...
  if (ShouldResistFingerprinting(RFPTarget::FontVisibilityBaseSystem)) {
    ...
    if (nsRFPService::IsRFPPrefEnabled(isPrivate)) {
      // profileMode: when profileMode is active, return User level so that
      // all installed fonts pass the visibility level check. The actual
      // per-container font filtering is done by IsFontAllowedByProfile,
      // which checks the font family name against the Profile's device.fontSet.
      if (StaticPrefs::privacy_fingerprint_profileMode()) {
        return FontVisibility::User;
      }
