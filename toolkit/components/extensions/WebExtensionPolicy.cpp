...
bool MozDocumentMatcher::Matches(const DocInfo& aDoc,
  ...
  // profileMode: only allow extensions listed in the container's
  // allowedExtensions whitelist to inject content scripts.
  if (loadContext && mExtension &&
      mozilla::StaticPrefs::privacy_fingerprint_profileMode()) {
    OriginAttributes docShellAttrs;
    loadContext->GetOriginAttributes(docShellAttrs);
    uint32_t userContextId = docShellAttrs.mUserContextId;
    if (userContextId != 0) {
      ProfileArgs profile;
      if (WindowGlobalChild::GetProfileForUserContextId(userContextId,
                                                        &profile)) {
        nsAutoCString extensionId;
        mExtension->Id()->ToUTF8String(extensionId);
        bool found = false;
        for (const auto& allowed : profile.allowedExtensions()) {
          if (extensionId.Equals(allowed)) {
            found = true;
            break;
          }
        }
