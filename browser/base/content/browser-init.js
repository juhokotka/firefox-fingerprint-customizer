...
var gBrowserInit = {
  ...
  _delayedStartup() {
      ...
      if (existingBtn) {
        ...
        if (navBarTarget && urlbarContainer) {
          ...
          function buildContainerPanel(panel) {
            ...
            // Determine which container the active tab belongs to.
            let activeTab = gBrowser.selectedTab;
            let activeUcId = activeTab ? activeTab.userContextId : 0;
            ...
            // ── Clear Container button (only when in container mode) ──
                ...
                try {
                  ...
                  if (profile && profile.device) {
                    ...
                    if (!devType) {
                      ...
                      if (lowerName.startsWith("macbook") || lowerName.startsWith("mac")) {
                        devType = "MacBook";
    ...
    if (containersMenuItem) {
      ...
      // Keep the button label in sync when the user switches tabs, so it
      // always shows the active tab's container name.
