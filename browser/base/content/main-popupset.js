...
document.addEventListener(
  ...
  () => {
    ...
    // Privacy Mode toggle — switches privacy.fingerprint.profileMode on/off.
    // When on, containers are also enabled (privacy mode = container isolation
    // with fingerprint profiles). When off, both are disabled (normal Firefox).
    const indicatorMenu = document.getElementById("userContext-indicator-menu");
    const privacyModeToggle = document.getElementById("privacyModeToggle");
    indicatorMenu.addEventListener("popupshowing", () => {
      privacyModeToggle.checked = Services.prefs.getBoolPref(
        "privacy.fingerprint.profileMode"
      );
    });
    privacyModeToggle.addEventListener("command", () => {
      let enabled = privacyModeToggle.checked;
      Services.prefs.setBoolPref("privacy.fingerprint.profileMode", enabled);
      Services.prefs.setBoolPref("privacy.userContext.enabled", enabled);
      // Toggle the File menu "New Container Tab" item visibility.
      let menuNewUserContext = document.getElementById("menu_newUserContext");
      if (menuNewUserContext) {
        menuNewUserContext.hidden = !enabled;
      }
      // When Privacy Mode turns off and the current tab isn't in a container,
      // hide the URL-bar indicator (it was only shown for the toggle).
      if (!enabled) {
        let userContextId =
    ...
    document
      .getElementById("context_switchContainerPopupMenu")
      .addEventListener("command", event => {
        TabContextMenu.switchContainer(event);
      });
    ...
    mainPopupSet.addEventListener("popupshowing", event => {
      switch (event.target.id) {
        ...
        case "context_reopenInContainerPopupMenu":
          ...
          break;
        case "context_switchContainerPopupMenu":
          TabContextMenu.createSwitchContainerMenu(event);
          break;
