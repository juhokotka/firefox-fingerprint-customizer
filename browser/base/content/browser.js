...
ChromeUtils.defineESModuleGetters(this, {
  ...
  ContainersPanelController:
    "resource:///modules/ContainersPanelController.sys.mjs",
  ContentAnalysis:
...
var gContainerCreation = {
  ...
  async open() {
    ...
    let onCreate = () => {
      this._editor.commit().catch(e => console.error(e));
      panel.hidePopup();
    };
...
var ContainerStatusUI = {
  _updateButtonStatus() {
    try {
      ...
      if (!containerMode) {
        if (dot) { dot.className = "container-status-dot off"; }
        if (text) { text.value = "Privacy Container: Off"; }
        return;
      }

      // Container mode is on — show which container the active tab belongs to.
      let label = "Privacy Container: On";
      try {
        let tab = gBrowser && gBrowser.selectedTab;
        let ucId = tab ? tab.userContextId : 0;
        console.log("[ContainerStatusUI] Updating button: tab userContextId =", ucId);
        if (ucId > 0) {
          let identity = ContextualIdentityService.getPublicIdentityFromId(ucId);
          console.log("[ContainerStatusUI] Identity lookup:", identity ? identity.name : "null");
          if (identity && identity.name) {
            label = "Privacy Container: " + identity.name;
          }
        }
      } catch (e2) {
        console.error("[ContainerStatusUI] Failed to get identity:", e2);
      }
      if (dot) { dot.className = "container-status-dot on"; }
      if (text) { text.value = label; }
      console.log("[ContainerStatusUI] Button label set to:", label);
    } catch (e) {
...
/**
 * Synchronizes fingerprint profiles to content processes.
 *
 * The parent-side ParentProfileCache (in WindowGlobalParent.cpp) is in-memory
 * only and starts empty on every browser launch.  This object ensures that:
 *  1. On startup, profiles for all containers are re-pushed to any open tabs.
 *  2. When a new container tab is opened, the profile is pushed to its
 *     WindowGlobalParent, which also caches it for future tabs in the same
 *     container.
 */
var ContainerProfileSync = {
  /**
   * Push the profile for a container to a specific tab's content process.
   * Called right after gBrowser.addTab() in the container panel click handler
  ...
  _pushProfile(bc, profile) {
    ...
    // WindowGlobal not ready yet — retry after the browser has had time to
    // create it.
    if (bc) {
      let attempts = 0;
  ...
  /**
   * Re-push profiles for all containers to all open container tabs.
