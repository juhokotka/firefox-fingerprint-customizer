...
var TabContextMenu = {
  ...
  updateContextMenu(aPopupMenu) {
    ...
    let containersEnabled =
      ...
      Services.prefs.getBoolPref("privacy.fingerprint.profileMode", false);
    reopenInContainer.hidden =
      !containersEnabled || PrivateBrowsingUtils.isWindowPrivate(window);
    reopenInContainer.disabled = this.contextTab.hidden;

    // "Switch Container" is only relevant when the context tab itself is in a
    // container — it closes all tabs of that container and reopens the page
    // in the chosen target container.
    let switchContainer = document.getElementById("context_switchContainer");
    let currentUci = parseInt(
      this.contextTab.getAttribute("usercontextid") || "0"
    );
    switchContainer.hidden =
      !containersEnabled ||
      PrivateBrowsingUtils.isWindowPrivate(window) ||
      currentUci == 0;
    switchContainer.disabled = this.contextTab.hidden;
  ...
  createSwitchContainerMenu(event) {
    createUserContextMenu(event, {
      isContextMenu: true,
      excludeUserContextId: this.contextTab.getAttribute("usercontextid"),
    });
  ...
  /**
   * Close all tabs of the context tab's container and reopen the current page
   * in the chosen target container. Shows a confirmation prompt first.
   */
  async switchContainer(event) {
    let targetUserContextId = parseInt(
      event.target.getAttribute("data-usercontextid")
    );
    let tab = this.contextTab;
    let currentUserContextId = parseInt(
      tab.getAttribute("usercontextid") || "0"
    );

    if (!currentUserContextId || currentUserContextId == targetUserContextId) {
      return;
    }

    // Gather container names and tab count for the confirmation dialog.
    let fromName = ContextualIdentityService.getUserContextLabel(
      currentUserContextId
    );
    let toName = ContextualIdentityService.getUserContextLabel(
      targetUserContextId
    );
    ...
    let [title, msg, okButton] = await document.l10n.formatValues([
      { id: "tab-context-switch-container-confirm-title" },
      {
        id: "tab-context-switch-container-confirm-msg",
        args: { count, from: fromName, to: toName },
      },
      { id: "tab-context-switch-container-confirm-button" },
    ]);
    ...
    // Capture the URL and triggering principal before closing the tab.
    ...
    // Close all tabs of the current container, then open the page in the
    // target container.
    ...
    Glean.containers.tabAssignedContainer.record({
      from_container_id: currentUserContextId,
      to_container_id: targetUserContextId,
    });
  },
