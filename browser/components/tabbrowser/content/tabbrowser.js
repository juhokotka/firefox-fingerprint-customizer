...

{
  ...
  function updateUserContextUIIndicator() {
    ...
    if (!userContextId) {
      ...
      // When Privacy Mode is active, keep the indicator visible even for
      // non-container tabs so the user can always access the toggle.
      if (
        Services.prefs.getBoolPref("privacy.fingerprint.profileMode", false)
      ) {
        replaceContainerClass("color", hbox, "");
        let indicator = document.getElementById("userContext-indicator");
        replaceContainerClass("icon", indicator, "fingerprint");
