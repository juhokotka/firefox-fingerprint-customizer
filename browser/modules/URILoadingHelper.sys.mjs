...
export const URILoadingHelper = {
  ...
  openLinkIn(window, url, where, params) {
    ...
    // Privacy Browser: when no explicit container is specified and
    // container mode is active, inherit the sticky selected container
    // so that all new tabs/navigations stay in the user's selection.
    if (userContextId == null) {
      try {
        if (Services.prefs.getBoolPref("privacy.browser.containerMode", false)) {
          let selected = Services.prefs.getIntPref(
            "privacy.browser.selectedUserContextId", 0
          );
          if (selected > 0) {
            userContextId = selected;
            params.userContextId = selected;
          }
        }
